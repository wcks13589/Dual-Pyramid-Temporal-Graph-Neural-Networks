...
class DualGNN_pyramid(nn.Module):
    def __init__(self, model_parameters):
        super(DualGNN_pyramid, self).__init__()
        ...
        self.gnn2 = nn.ModuleList([TemGNN(model_parameters, model_parameters.window_size-i) for i in range(model_parameters.window_size)])
        self.att_w1 = nn.Linear(model_parameters.rnn_hidden_dim, 1)
        self.att_w2 = nn.Linear(model_parameters.rnn_hidden_dim, 1)
    ...
    def forward(self, *x):
        ...
        embed2 = self.pyramid(static_dense_x, static_sparse_x, dynaminc_dense_x, dynamic_sparse_x, edges, weights, self.gnn2, self.att_w2)
        
        state = self.state_decoder(embed2).sigmoid()
